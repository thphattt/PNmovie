...
//Inngest function to send email when user booking a show
const sendBookingConfirmationEmail = inngest.createFunction(
  ...
  <p>
      ...
      ).toLocaleDateString("en-US", { timeZone: "Asia/Ho_Chi_Minh" })}<br/>
      <strong>Time:</strong> ${new Date(
        booking.show.showDateTime,
      ).toLocaleTimeString("en-US", { timeZone: "Asia/Ho_Chi_Minh" })}
  </p>
  <p>Enjoy the show! 🍿</p>
  ...
  async ({ step }) => {
    ...
    const results = await step.run("send-all-reminders", async () => {
      return await Promise.allSettled(
        reminderTasks.map((task) =>
          sendEmail({
            ...
            body: `<div style="font-family: Arial, sans-serif; padding: 20px;">
                  ...
                  <p> is scheduled for <strong>${new Date(task.showTime).toLocaleDateString("en-US", { timeZone: "Asia/Ho_Chi_Minh" })}</strong> at
        ...
        <br/>
        <p>Enjoy the show!<br/>QuickMovie Team</p>
</div>`,
          }),
        ),
      );
